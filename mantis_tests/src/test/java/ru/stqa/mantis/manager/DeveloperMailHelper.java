...
import okhttp3.*;
import ru.stqa.mantis.manager.developermail.AddUsersResponse;
...
public class DeveloperMailHelper extends HelperBase{
    ...
    public DeveloperMailUser addUser() {
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            var addUsersResponse = new ObjectMapper().readValue(text, AddUsersResponse.class);
            if (!addUsersResponse.succes()) {
                throw new RuntimeException(addUsersResponse.errors().toString());
            }
            return addUsersResponse.result();
        } catch (IOException e) {
